...
source: https://docs.axway.com/bundle/axway-open-docs/page/docs/apim_policydev/apigw_gw_instances/configure_http_strict_transport_security/index.html
scrapedAt: 2026-08-21T20:45:34.786Z
---
...
3. Configure the following settings: Name: Unique name of the HSTS profile. Defaults to HTTP Strict Transport Security. Enable HSTS: Specifies whether HSTS processing is enabled for the profile. Defaults to enabled. HSTS Parameters: max-age (seconds): Specifies the time, in seconds, that the browser should remember that a site is only to be accessed using HTTPS. includeSubDomains: Applies the new profile to all of the site’s subdomains. Defaults to enabled.

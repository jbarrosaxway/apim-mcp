...
source: https://docs.axway.com/bundle/axway-open-docs/page/docs/apim_policydev/apigw_polref/encryption_xml/index.html
scrapedAt: 2026-08-21T20:48:00.524Z
---
...
Encryption Algorithm: The Encryption algorithm selected is used to encrypt the data. GCM algorithms are recommended for better security. The following algorithms are available:
...
Key Wrap Algorithm: The Key Wrap algorithm selected is used to wrap (encrypt) the symmetric encryption key with the recipient’s public key. The following key wrap algorithms are available:
...
MGF1 Algorithm: When using the KwRsaOaepMGF1, you must select a Digest option from the Mask Generation Function drop-down list. The following digests are available:
...
Algorithm Suite Required: Select the WS-Security Policy Algorithm Suite that must be used when encrypting the message. This check ensures that the appropriate algorithms were used to encrypt the message. When this option is not selected, the algorithms specified within the encrypted document are used. Where possible, it is recommended to select an Algorithm Suite.
...
- XML encryption settings filter XML encryption overview Elements Asymmetric and symmetric cryptography Encryption key settings Key info settings Recipient settings What to encrypt settings Advanced XML encryption settings Auto-generation using the XML encryption settings wizard

- XML decryption settings filter Nodes to decrypt Decryption key Options Auto-generation using the XML decryption settings wizard

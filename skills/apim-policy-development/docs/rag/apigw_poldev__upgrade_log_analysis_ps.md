...
source: https://docs.axway.com/bundle/axway-open-docs/page/docs/apim_policydev/apigw_poldev/upgrade_log_analysis_ps/index.html
scrapedAt: 2026-08-21T20:44:32.246Z
---
...
1. Open the problematic API Gateway configuration in Policy Studio. For example, in Policy Studio version 7.7, create a new project using From .fed file or From .pol and .env files, and select the correct files under the following directory: /opt/Axway-7.7/apigateway/upgrade/bin/out/upgrade/esgroups/groups/group-2/ Copy

2. Select Tools > Upgrade Log Analysis from the main menu.

3. Select the upgrade.log file generated by sysupgrade, for example: /opt/Axway-7.7/apigateway/upgrade/bin/out/log/upgrade.log Copy

4. Click Open.

5. If the log file contains data on multiple groups, select the group configuration to analyze in the dialog. The upgrade.log file generated by sysupgrade can contain upgrade information for multiple groups, whereas the automatic upgrades triggered by Policy Studio are specific to the project upgraded.
...
A warning appears if an Amazon Web Services filter without a setting for either AWS Credential or Client settings is found.
...
- Example upgrade log analysis Upgrade indicators in Policy Studio tree View more details in upgrade log Restore completed issues

- Example critical/major issues and recommended solutions Discarded web service Web Service error: Failed to find binding for operation Migrated resolver for web service Discarded Contivo filter

- Example warnings and recommended solutions Fastest scripting language not used ActiveMQ path update Default SSL ciphers/options not used Amazon Web Services filters API Gateway KPS collection KPS table PortalExternalClientStore missing OAuth resource owner login OAuth services interface OAuth KPS definitions missing OAuth authorizations table missing

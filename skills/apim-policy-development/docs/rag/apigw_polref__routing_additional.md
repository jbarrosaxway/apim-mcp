...
source: https://docs.axway.com/bundle/axway-open-docs/page/docs/apim_policydev/apigw_polref/routing_additional/index.html
scrapedAt: 2026-08-21T20:47:16.111Z
---
...
HTTP response code status: Enter the status code returned to the client. For a complete list of status codes, see the HTTP specification.
...
Cc: Enter the email address of the Cc (carbon copy) recipients of the messages. You can enter multiple addresses by separating each one using a semicolon.

Bcc: Enter the email address of the Bcc (blind carbon copy) recipients of the messages. You can enter multiple addresses by separating each one using a semicolon.
...
For details on how to configure a packet sniffer, see Packet sniffers.
...
This filter is used by Management Services that are configured to listen on the Management Interface on port 8090. For more information on how the Call internal service filter is used by these services, see Management services.
...
- HTTP redirect filter Configure a HTTP redirect

- HTTP status filter Configure a HTTP status

- File upload filter Configure a file upload Configure upload connection

- File download filter Configure a file download Configure file download connection

- Extract path parameters filter Configure extract path parameters Required input and generated output Possible outcomes

- SMTP filter Configure message settings

- Insert WS-Addressing filter Configure WS-Addressing information to insert

- Read WS-Addressing filter Configure WS-Addressing information to read

- Save to file filter

- TIBCO Rendezvous filter Configure routing to TIBCO Rendezvous Add fields

- Wait for response packets filter Response packet sniffing

- Call internal service filter Configure call internal service

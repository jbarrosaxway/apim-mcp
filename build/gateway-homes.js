...
function parseConfigJson(raw, source) {
    ...
    if (Array.isArray(parsed)) {
        return { mappings: parsed, defaultGatewayHome: null };
    }
    const obj = parsed;
    return {
...
function lookupByProductVersion(config, productVersion) {
    const matches = config.mappings.filter((m) => m.productVersion?.trim() === productVersion.trim());
    if (matches.length === 0)
        return undefined;
    return pickExistingHome(matches.map((m) => m.gatewayHome));
}
async function fetchProductVersion(api, instanceId) {
    const response = await api.listTopology();
    const rawTopology = response.result;
    if (!rawTopology?.productVersion) {
        throw new Error("Topology response missing productVersion — cannot resolve gatewayHome from instanceId.");
    }
    if (instanceId) {
        const groups = rawTopology.groups || [];
        const instances = groups.flatMap((g) => g.services || []);
        const found = instances.some((s) => s.id === instanceId);
        if (!found) {
            throw new Error(`instanceId '${instanceId}' not found in topology. Call axway_apim_topology_list for current IDs.`);
        }
        return { productVersion: rawTopology.productVersion, instanceId };
    }
    return { productVersion: rawTopology.productVersion };
}
function libUnavailableMessage(productVersion) {
    if (productVersion) {
        return (`No gatewayHome mapping for productVersion ${productVersion}. Offline checks only (Tier 0). ` +
            `Configure ${CONFIG_REL}, set AXWAY_GATEWAY_VERSION_MAP / AXWAY_GATEWAY_HOME, or pass gatewayHome.`);
    }
    return ("No gatewayHome resolved. Tier 1 (Axway libs) unavailable. " +
        `Configure ${CONFIG_REL}, set AXWAY_GATEWAY_VERSION_MAP / AXWAY_GATEWAY_HOME, or pass gatewayHome.`);
}
/**
 ...
 */
export async function resolveGatewayHome(api, args = {}) {
    ...
    if (instanceId) {
        if (!api) {
            const msg = "instanceId requires live Gateway topology (AXWAY_GATEWAY_* credentials). " +
...
//# sourceMappingURL=gateway-homes.js.map
 ...
 */
export const TOOL_REQUIRED_PROFILE = {
...
export function resolveEffectiveProfile(scopes) {
    let best = null;
    for (const scope of scopes) {
        let profile = null;
        if (scope === PROFILE_SCOPES.observe)
            profile = "observe";
        else if (scope === PROFILE_SCOPES.operator)
            profile = "operator";
        else if (scope === PROFILE_SCOPES.admin)
            profile = "admin";
        else if (scope === LEGACY_ADMIN_SCOPE)
            profile = "admin";
        if (!profile)
            continue;
...
export function profileCovers(have, required) {
    if (!have)
        return false;
    return PROFILE_RANK[have] >= PROFILE_RANK[required];
}
export function requiredProfileFor(toolName) {
    return TOOL_REQUIRED_PROFILE[toolName] ?? "admin";
}
export function requiredScopeFor(toolName) {
    return PROFILE_SCOPES[requiredProfileFor(toolName)];
}
export function toolAllowed(toolName, scopesOrProfile) {
    const required = requiredProfileFor(toolName);
    if (scopesOrProfile === null)
        return false;
...
export function scopeSatisfied(have, need) {
    if (have.includes(need))
        return true;
    const needProfile = need === PROFILE_SCOPES.observe
        ? "observe"
...
export function toolsAllowedFor(toolNames, scopesOrProfile) {

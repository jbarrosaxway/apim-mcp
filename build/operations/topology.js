...
async function resolveInstanceId(api, instanceId) {
    if (instanceId)
        return instanceId;
    const response = await api.listTopology();
    const groups = response.result?.groups || [];
    const instances = groups.flatMap((group) => (group.services || []).map((service) => ({
    ...
    if (instances.length > 1) {
        const list = instances.map((i) => `${i.instanceName} (${i.instanceId})`).join(", ");
 ...
 */
export async function getDeploymentArchive(api, instanceId, savePath, returnBase64) {
...
function packageBinaryArchive(opts) {
    ...
    if (!b64 || typeof b64 !== "string") {
        throw new Error(`${opts.kind} archive response missing result.data (base64 payload).`);
    }
    ...
    const inlineAllowed = includeBase64 && bytes.length <= MAX_INLINE_FED_BASE64_BYTES;
    return {
        ...opts.identity,
        kind: opts.kind,
...
async function resolveArchiveTarget(api, args) {
...
export async function getPolicyArchive(api, args) {
    try {
        const target = await resolveArchiveTarget(api, args);
        const response = target.mode === "service"
        ...
        const endpoint = target.mode === "service"
...
export async function getEnvironmentArchive(api, args) {
    try {
        const target = await resolveArchiveTarget(api, args);
        const response = target.mode === "service"
...
export async function getEnvSettings(api, args) {
    try {
        const target = await resolveArchiveTarget(api, args);
        const response = target.mode === "service"
 ...
 */
export async function getGroupConfFile(api, groupId, filename, savePath, returnBase64) {
    ...
    try {
        ...
        const inlineAllowed = includeBase64 && bytes != null && bytes.length <= MAX_INLINE_FED_BASE64_BYTES;
        return {
            groupId,
            filename,
            endpoint,
...
function throwEmtHint(error, what, fallbackTool) {
    if (isEmtBlocked(error)) {
        throw new Error(`Cannot download ${what}: Externally Managed Topology (EMT) blocks this Deployment API method (HTTP 405). ` +
...
export async function listDomainDeployments(api) {
    try {
        ...
        return {
            ...
            message: "Domain deployment details retrieved. archiveID is typically rootProperties.Id on each instance entry.",
            relatedTools: [
                {
                    tool_name: "axway_apim_deployment_archive_get",
        ...
        const instances = (topo.groups || []).flatMap((g) => (g.instances || []).map((inst) => ({
            groupId: g.groupId,
        ...
        for (const inst of instances) {
            try {
                ...
                const res = await api.getGateway(`/router/service/${encoded}/api/configuration/archiveId`);
                deployments.push({
        ...
        return {
            ...
            emtNote: "GET /deployment/domain/deployments returned 405 (Externally Managed Topology). Fell back to per-instance archiveId via Configuration API.",
            deployments,
            message: "EMT domain: archive IDs collected per instance via Routing/Configuration API. Policy/env/group-conf Deployment API downloads are typically also blocked under EMT — use axway_apim_deployment_archive_get (.fed) instead.",
            relatedTools: [
                {
                    tool_name: "axway_apim_deployment_archive_get",

...
source: https://docs.axway.com/bundle/axway-open-docs/page/docs/apim_policydev/apigw_kps/configure_database_storage/index.html
scrapedAt: 2026-08-21T20:47:00.506Z
---
...
1. Edit the following file: INSTALL_DIR/apigateway/system/lib/log4j2.xml Copy

2. Add the following settings: Change the level from "error" to "debug" in all the lines shown below: <Root level="info"> <Logger name="org.apache.openjpa.Tool" level="error" additivity="false"> <Logger name="org.apache.openjpa.Runtime" level="error" additivity="false"> <Logger name="org.apache.openjpa.Remote" level="error" additivity="false"> <Logger name="org.apache.openjpa.DataCache" level="error" additivity="false"> <Logger name="org.apache.openjpa.MetaData" level="error" additivity="false"> <Logger name="org.apache.openjpa.Enhance" level="error" additivity="false"> <Logger name="org.apache.openjpa.Query" level="error" additivity="false"> <Logger name="org.apache.openjpa.jdbc.SQL" level="error" additivity="false"> <Logger name="org.apache.openjpa.jdbc.SQLDiag" level="error" additivity="false"> <Logger name="org.apache.openjpa.jdbc.JDBC" level="error" additivity="false"> <Logger name="org.apache.openjpa.jdbc.Schema" level="error" additivity="false"> Copy
...
1. Edit the following file: INSTALL_DIR/apigateway/system/conf/log4j2.xml Copy
...
3. Enter a database-specific JDBC SQL query to retrieve the required data. For example: select * from User where email = ? Copy
...
1. On the Properties tab in the Query field, enter the following: select * from User where firstName = ? and lastName = ? Copy For example:
...
For details on installing and configuring Apache Cassandra HA for API Gateway and API Manager, see Configure a highly available Cassandra cluster.

For details on adding an Cassandra database data store to a selected KPS collection, see Add a Cassandra data store.
...
- Shared database storage Create a KPS database table Set up an external connection to the database Use the external connection in a KPS collection Database storage information Logging for shared table storage

- Per-table database storage Map a database table using a single key Set up external connection to the database Add the database data source to the KPS collection Map the SQL table to a KPS table Define the KPS table structure Define the policy and path Deploy and run How to map a database table using a composite key Modify the KPS table Modify the policy Deploy and run the policy

- Configure Apache Cassandra KPS storage

- Configure file-based KPS storage Configure a file-based KPS collection

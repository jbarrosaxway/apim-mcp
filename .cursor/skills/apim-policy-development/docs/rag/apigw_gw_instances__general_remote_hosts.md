...
source: https://docs.axway.com/bundle/axway-open-docs/page/docs/apim_policydev/apigw_gw_instances/general_remote_hosts/index.html
scrapedAt: 2026-08-21T20:45:08.214Z
---
...
You can use the Remote Host Settings to override incoming connection parameters (see Configure an incoming remote host).
...
2. If the address fails to send the message, it ignores that address for a period of time and selects another address in the same way. The default period of time that an address is ignored by the load balancing algorithm is 60 seconds. You can change this time by setting the AXWAY_LB_ALG_ADDR_DOWNTIME user environment variable to an appropriate number of seconds.
...
1. Configure an HTTP Watchdog for the remote host. See Configure HTTP watchdog.

2. Configure a Requires Endpoint condition on the HTTP interface. See Configure conditions for an HTTP Interface.
...
- Configure watchdogs Configure HTTP watchdog

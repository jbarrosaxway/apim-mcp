...
source: https://docs.axway.com/bundle/axway-open-docs/page/docs/apim_policydev/apigw_kps/kps_configuration/index.html
scrapedAt: 2026-08-21T20:46:56.896Z
---
...
2. Specify the following settings in the dialog: Name: A collection must have a unique name in the API Gateway group. Description: You can provide an optional description. Alias prefix: You can also specify an alias prefix, but in normal usage, you can leave this field blank. Default data source: A collection has a default data source where all data for all tables in the collection is stored. You can change this data source or assign a different data source to individual tables in the collection.
...
2. Specify the following settings in the dialog: Name: A KPS table must have a unique name in the collection. Description: You can provide an optional description. Override the default data source with the following: You can specify a different data source than the collection if required. When the table is created, if required, you can use the Override the default data source with the following setting to specify a different data source than the collection:
...
A KPS collection has one active data source associated with it. All tables in the collection use this data source by default. You can configure a table to use a different data source if required (see Configure a KPS table).
...
For an example of accessing KPS data from a selector using a primary key, see Get started with KPS.

For examples of selectors that use both primary and composite keys, see Configure database storages.
...
- KPS table structure Edit the structure of an existing KPS table

- Query tables using properties and keys Primary key Secondary key Selector access Selector syntax Selector examples Auto-generated properties Encrypted properties

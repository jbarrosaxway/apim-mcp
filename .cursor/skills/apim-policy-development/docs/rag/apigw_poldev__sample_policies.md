...
source: https://docs.axway.com/bundle/axway-open-docs/page/docs/apim_policydev/apigw_poldev/sample_policies/index.html
scrapedAt: 2026-08-21T20:44:13.317Z
---
...
- Security: Verifies the digital signature on the request and creates a signature on the response. Decrypts the request and encrypts part of the response.
...
- Set up the sample policies Enable the sample services interface Configure a different sample services interface StockQuote demo service Remote host settings

- Conversion sample policy REST to SOAP policy Run the conversion sample

- Security sample policies Signature verification Run the signature verification sample Encryption and decryption Run the encryption and decryption sample

- Throttling sample policy Throttling policy Run the throttling sample

- Virtualized service sample policy Virtualized service policies Content-based routing policies Response transformation policy Run the virtualized service sample

- Stress test with send request sr Basic sr command examples Advanced sr command examples sr command arguments

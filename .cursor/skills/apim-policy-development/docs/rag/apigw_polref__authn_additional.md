...
source: https://docs.axway.com/bundle/axway-open-docs/page/docs/apim_policydev/apigw_polref/authn_additional/index.html
scrapedAt: 2026-08-21T20:47:28.484Z
---
...
Because the WS-Utility timestamp is inserted into the WS-Security header block, it is also referred to as a WSS timestamp. For example, see Extract WSS timestamp.
...
To add an agent, right-click the SiteMinder/SOA Security Manager Connections tree node, and select Add a SOA Security Manager Connection. Alternatively, you can add SOA Security Manager connections under the Environment Configuration > External Connections node in the Policy Studio tree. For details on how to configure SOA Security Manager connections, see Configure SiteMinder/SOA Security Manager connections.
...
3. Add the following line, where token_age_limit specifies the token age limit in minutes: WS_UT_CREATION_EXPIRATION_MINUTES=token_age_limit Copy
...
- SAML PDP authentication filter Configure request settings SAML subject settings Subject confirmation settings Configure response settings

- CA SOA Security Manager authentication filter Prerequisites Add third-party binaries to API Gateway Add third-party binaries to Policy Studio Configure CA SOA Security Manager authentication Message details settings XmlToolkit.properties file Configure the user name and password digest token age restriction

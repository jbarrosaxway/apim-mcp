...
source: https://docs.axway.com/bundle/axway-open-docs/page/docs/apim_policydev/apigw_polref/attributes_retrieve/index.html
scrapedAt: 2026-08-21T20:47:24.301Z
---
...
- Unmarshal as: Enter the data type to unmarshal the message attribute value as (defaults to java.util.List). For example, typical data types to unmarshal as include the following: java.lang.String: Enter this value when using this filter to extract a single value and store it in an API Gateway attribute on the message whiteboard. com.fasterxml.jackson.databind.JsonNode: Enter this value when using this filter to store the content in a message attribute and then place it in a JSON message using a JSON Add Node filter.
...
Use the following XPath: Configure an XPath expression to retrieve the desired content. Click the Add button to add an XPath expression. You can add and remove existing expressions by clicking the Edit and Remove buttons respectively. For more information, see Configure XPath expressions.
...
- Retrieve attributes with JSON path filter Configure retrieve attributes with JSON path JSON Path examples Retrieving attributes Retrieving multiple attributes in a list Exceptions

- Retrieve from HTTP header filter

- Retrieve attribute from SAML attribute assertion filter Configure details settings Configure trusted issuer settings Configure subject settings Configure lookup attributes settings

- Retrieve attribute from SAML PDP filter Configure request settings SAML subject settings Subject confirmation settings Attributes Configure response settings

- Retrieve attribute from Tivoli filter

- Retrieve attribute from message filter

- Retrieve attribute from database filter Configure database settings Configure advanced settings

- Retrieve attribute from directory server filter Configure database settings for directory server Configure advanced settings for directory server

- Retrieve attribute from user store filter Configure database settings for user store Configure advanced settings for user store

...
source: https://docs.axway.com/bundle/axway-open-docs/page/docs/apim_policydev/apigw_gw_instances/general_websocket/index.html
scrapedAt: 2026-08-21T20:45:11.768Z
---
...
2. If the server is capable and willing to upgrade the connection, it sends a HTTP 101 response to the requesting client. At this point the handshake is considered successful and the connection between the server and the client is upgraded to the WebSocket protocol. As soon as the client receives the HTTP 101 response the connection is no longer considered an HTTP connection.
...
2. Click the WebSockets tab. A view of all WebSocket frames sent from or received by API Gateway is displayed. A message might consist of one or more frames.
...
- Configure a WebSocket connection WebSocket configuration settings Policies settings Advanced settings CORS settings

- Monitor a WebSocket connection

- WebSocket connection example Create a HTTP/1.1 remote host Create a policy to handle the Upgrade request Create policies to handle data frames Create a WebSocket handler Test the WebSocket connection

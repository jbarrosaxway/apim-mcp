...
export type FragmentCleanupResult = {
  enabled: boolean;
  deletedFiles: string[];
  freedBytes: number;
  skipped: string[];
...
export type FragmentCleanupConfig = {
  enabled: boolean;
  maxAgeMs: number;
  intervalMs: number;
  maxXmlBytesPerPackage?: number;
};
...
export function loadFragmentCleanupConfig(): FragmentCleanupConfig {
  return {
    enabled: parseBool(process.env.FRAGMENT_CLEANUP_ENABLED, true),
    maxAgeMs: parsePositiveInt(
      process.env.FRAGMENT_ARTIFACT_MAX_AGE_MS,
      DEFAULT_MAX_AGE_MS
    ),
    intervalMs: parsePositiveInt(
      process.env.FRAGMENT_CLEANUP_INTERVAL_MS,
      DEFAULT_INTERVAL_MS
    ),
    maxXmlBytesPerPackage: parseOptionalPositiveInt(
      process.env.FRAGMENT_MAX_XML_BYTES_PER_PACKAGE
    ),
  };
}

function isYamlesTrace(name: string): boolean {
  return name === "yamles.trc" || /^yamles\.trc\.\d+$/.test(name);
}

function isMcpTestXml(name: string): boolean {
...
function collectXmlFiles(xmlDir: string): { path: string; mtimeMs: number; size: number }[] {
...
function purgeXmlDir(
  xmlDir: string,
  maxAgeMs: number,
  maxXmlBytes: number | undefined,
  now: number,
  result: FragmentCleanupResult
): void {
  ...
  if (total <= maxXmlBytes) return;

  remaining.sort((a, b) => a.mtimeMs - b.mtimeMs);
  for (const file of remaining) {
    if (total <= maxXmlBytes) break;
 ...
 */
export function cleanupFragmentArtifacts(
  config: FragmentCleanupConfig = loadFragmentCleanupConfig()
): FragmentCleanupResult {
  const result: FragmentCleanupResult = {
...
function logCleanupSummary(result: FragmentCleanupResult, context: string): void {
  if (!result.enabled) return;
  if (result.deletedFiles.length === 0 && result.errors.length === 0) return;
  const mb = (result.freedBytes / (1024 * 1024)).toFixed(2);
  console.error(
    `[fragment-cleanup] ${context}: removed ${result.deletedFiles.length} file(s), freed ~${mb} MiB` +
      (result.errors.length > 0 ? `, ${result.errors.length} error(s)` : "")
  );
}

/** Run cleanup once (non-blocking). */
export function runFragmentCleanupAsync(context = "scheduled"): void {
  ...
  setImmediate(() => {
    try {
      ...
      console.error(
        `[fragment-cleanup] ${context} failed:`,
...
export function cleanupAfterFragmentOperation(): void {
 ...
 */
export function startFragmentCleanupScheduler(): void {
  const config = loadFragmentCleanupConfig();
  if (!config.enabled) {
  ...
  if (config.intervalMs > 0) {
    ...
    console.error(
      `[fragment-cleanup] periodic purge every ${config.intervalMs}ms (max age ${config.maxAgeMs}ms)`
    );
  }
}

/** Test helper: stop periodic cleanup. */
export function stopFragmentCleanupScheduler(): void {
  if (schedulerTimer) {

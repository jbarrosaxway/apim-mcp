...
export type FragmentPackageInfo = {

  relativePath: string;

  absolutePath: string;

  hasYamlFragment: boolean;
...
export function findRepoRoot(): string {
  ...
  for (const candidate of candidates) {
    ...
    if (
      ...
      fs.existsSync(path.join(resolved, "build", "index.js")) ||
...
export function defaultFragmentPackage(repoRoot: string): string {

  const packages = scanFragmentPackages(repoRoot);

  const marked = packages.find((pkg) => pkg.default);

  if (marked) return marked.relativePath;
...
export function isValidFragmentPackage(packageRoot: string): boolean {

  const hasYamlFragment = fs.existsSync(path.join(packageRoot, "fragment"));

  const hasScripts = fs.existsSync(

    path.join(packageRoot, "scripts", "validate-fragment.py")
...
function packageHasXml(packageRoot: string): boolean {

  const xmlDir = path.join(packageRoot, "fragment-xml");

  if (!fs.existsSync(xmlDir)) return false;
...
export function scanFragmentPackages(repoRoot: string): FragmentPackageInfo[] {
  ...
  const packages: FragmentPackageInfo[] = [];
...
function isPoliciesRelativePath(requested: string): boolean {
  ...
  if (rel.startsWith("..") || path.isAbsolute(rel)) {
    ...
    if (paramName === "fragmentPath" && !isPoliciesRelativePath(requested)) {
      ...
      hints.push(
        ...
        "  hint: For local-only packages use axway_apim_fragment_validate_submit with files map, not fragmentPath."
    ...
    hints.push(
      ...
      "  hint: Use axway_apim_fragment_packages_list for whitelisted package paths under policies/."

    );

    hints.push(

      `  hint: Policy packages live in the separate ${EXTERNAL_POLICIES_REPO} repo — mount at /app/policies or use validate_submit.`
...
function formatPackageNotFoundError(
...
export function resolveFragmentPackageRoot(
...
export function resolveXmlOutputPath(
...
export function resolvePsProjectPath(

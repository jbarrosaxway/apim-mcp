...
export const GENERIC_VALIDATE_SCRIPT_REL =
  "resources/fragment/scripts/validate-fragment-generic.py";
...
export type FragmentSandboxWriteStats = {
  fileCount: number;
  totalBytes: number;
};
...
export function uploadMaxBytes(): number {
  return parsePositiveInt(
    process.env.FRAGMENT_UPLOAD_MAX_BYTES,
    DEFAULT_UPLOAD_MAX_BYTES
  );
}

export function uploadMaxFiles(): number {
  return parsePositiveInt(
    process.env.FRAGMENT_UPLOAD_MAX_FILES,
    DEFAULT_UPLOAD_MAX_FILES
  );
}

export function sandboxMaxAgeMs(): number {
  return parsePositiveInt(
    process.env.FRAGMENT_SANDBOX_MAX_AGE_MS,
    DEFAULT_SANDBOX_MAX_AGE_MS
  );
}

export function sandboxRootDir(repoRoot: string): string {
...
function normalizeRelativePath(relPath: string): string {
  ...
  if (!normalized || containsTraversalSegment(normalized)) {
    throw new FragmentPathSecurityError(
      `Fragment upload rejected (security): invalid relative path "${relPath}"`
    );
  }
  if (path.isAbsolute(normalized)) {
    throw new FragmentPathSecurityError(
      `Fragment upload rejected (security): absolute paths are not allowed: "${relPath}"`
    );
  }
  return normalized;
}

function assertWithinSandbox(
  sandboxRoot: string,
  targetPath: string,
  relPath: string
): void {
...
export function createSandboxDir(
  repoRoot: string,
  packageLabel?: string
): { sandboxId: string; absolutePath: string; relativePath: string; label?: string } {
...
export function writeSandboxFiles(
  ...
  if (entries.length > maxFiles) {
    throw new FragmentUploadError(
      `Upload rejected: ${entries.length} files exceeds limit of ${maxFiles}`
    );
  }
  ...
  for (const [relPath, content] of entries) {
    ...
    if (totalBytes > maxBytes) {
      throw new FragmentUploadError(
        `Upload rejected: total size exceeds ${maxBytes} bytes (FRAGMENT_UPLOAD_MAX_BYTES)`
      );
    }
...
function readTarHeader(buffer: Buffer, offset: number): {
  name: string;
  size: number;
  type: string;
  nextOffset: number;
...
function extractTarToSandbox(tarBuffer: Buffer, sandboxRoot: string): FragmentSandboxWriteStats {
  const maxBytes = uploadMaxBytes();
  const maxFiles = uploadMaxFiles();
  let totalBytes = 0;
  let fileCount = 0;
  ...
  while (offset < tarBuffer.length) {
    ...
    if (size > 0) {
      if (fileCount + 1 > maxFiles) {
        throw new FragmentUploadError(
          `Archive rejected: exceeds ${maxFiles} files (FRAGMENT_UPLOAD_MAX_FILES)`
        );
      }
      totalBytes += size;
      if (totalBytes > maxBytes) {
        throw new FragmentUploadError(
          `Archive rejected: exceeds ${maxBytes} bytes (FRAGMENT_UPLOAD_MAX_BYTES)`
        );
      }
...
export function extractArchiveToSandbox(
  sandboxRoot: string,
  ...
  if (raw.length > uploadMaxBytes()) {
    throw new FragmentUploadError(
      `Archive rejected: compressed size exceeds ${uploadMaxBytes()} bytes`
    );
  }
  ...
  try {
    ...
    throw new FragmentUploadError(
      "Failed to decompress archive — expected gzip-compressed tar (.tar.gz)"
    );
  }
...
export function bootstrapSandboxScripts(
  repoRoot: string,
  sandboxRoot: string
): string[] {
...
export function assertSandboxPackage(sandboxRoot: string): void {
...
export function purgeSandboxDir(sandboxPath: string): boolean {
  try {
...
export function purgeStaleSandboxes(
  maxAgeMs: number = sandboxMaxAgeMs()
): { purged: string[]; errors: string[] } {

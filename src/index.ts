...
class AxwayMcpServer extends McpServer {
  ...
  private registerApimResources() {
    ...
    this.resource(
      ...
      {
        description:
          "One scraped Axway Policy Development doc page as markdown (RAG). Slug matches filename under skills/apim-policy-development/docs/rag/ without .md. List via resources/list on this template; start from _manifest via slug index or playbook. Read-only offline mirror of docs.axway.com apim_policydev; re-crawl with crawl-policydev-docs.mjs to refresh.",
        mimeType: "text/markdown",
      },
      async (uri, variables) => {
    ...
    this.resource(
      "apim_policies_packages",
      "axway://apim/policies/packages",
      {
        description:
          "Index of Policy Studio configuration fragment packages discovered under policies/ on the MCP host.",
        mimeType: "application/json",
      },
      async (uri) => {

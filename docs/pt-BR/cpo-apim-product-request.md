...
Languages: [English](../en/cpo-apim-product-request.md) | [Português (Brasil)](cpo-apim-product-request.md)

**Para:** CPO / AETHER (owners do Axway MCP style guide e `axway-mcp-auditor`)  
**De:** equipe APIM MCP (`apim-mcp` / Axway API Gateway + API Manager)  
**Assunto:** Incluir `apim` no conjunto canônico de product segments (critério A10)

## Pedido

Adicionar **`apim`** à lista canônica de segmentos de produto usada em:
...
- Sem `apim` na lista, **todas** as tools geram finding **High A10**, bloqueando GA-ready apesar de compliance em outros critérios.
...
2. Critério A10 / style guide atualizados.
3. Release do auditor (patch/minor) com nota no CHANGELOG.
4. Equipes APIM re-executam `audit_manifest.py` e fecham o finding A10 documentado.

## Contato

Repositório: `apim-mcp` · naming atual: `axway_apim_*` · resources: `axway://apim/`.
